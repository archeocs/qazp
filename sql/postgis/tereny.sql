...
CREATE TABLE TEREN_DANE(
    ...
    nieuzytek varchar(1) check (laka in ('T','N')),
    torf varchar(1) check (laka in ('T','N')),
    woda varchar(1) check (laka in ('T','N')),
    bagno varchar(1) check (laka in ('T','N')), 
    ...
    utwor_geo varchar(1) check (utwor_geo in ('L','Z','T')), -- utwor geologiczny L - luzny, Z - zwiezly, T-torf-bag (AZPMAX piaszczysta, gliniasta, tofr-bag)
    kamienistosc varchar(1) check (kamienistosc in ('M','S','D')),
    specjalistyczne varchar(50),
    uwagi varchar(255),
