...
class Zagrozenie(object):
    czy_zm = False
    nowe = False
    modstr = "select * from azp_zmien_zagrozenie(%d,'%s','%s','%s','%s','%s','%s','%s','%s', %d)"
    ...
    def __init__(self,stan,con,td=None):
        if td is not None and len(td) == 9:
            zm = {'T':1,'N':0}
            self.istn, self.stal, self.doraz, self.ludz, self.nat = zm[td[1]],zm[td[2]],zm[td[3]],zm[td[4]],zm[td[5]]
            self.pryw,self.spol = zm[td[6]],zm[td[7]]
            self.dodat = td[8].decode('utf-8')
            self.zid = td[0]
        else:
    ...
    def zapisz(self):
        ...
        if self.czy_zm:
            zm = {1:'T',0:'N'}
            args = (self.stan.sid,zm[self.istn],zm[self.stal],zm[self.doraz],zm[self.ludz],zm[self.nat],
